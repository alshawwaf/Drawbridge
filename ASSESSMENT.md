...
control panel**, one app, **208 tests passing**, **live-confirmed against real R82.10 CloudGuard**.
...
| Gateways & DC creds | **AES-256-GCM encrypted at rest** (gateway password + datacenter credentials), cert-pin, real-gateway push; DC password copyable (masked) from the DC page | ✅ |
...
| **Diagnostics / infra** | Activity log (full request/response, redacted, per-provider filter + search), auth, Docker/Dokploy deploy, **208 tests** | ✅ |
