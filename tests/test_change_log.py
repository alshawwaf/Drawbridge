...
def test_record_skips_dry_runs_and_changes_without_inverse(db):
    # dry-run (published False) -> nothing committed -> nothing to roll back
    assert change_log.record(db, server=_Srv(), request=REQ, layer="L",
                             result={"ok": True, "published": False, "outcome": "create", "inverse": _INV}) is None
    # published but no inverse (e.g. the SMS didn't return a uid) -> can't be reverted -> not recorded
    assert change_log.record(db, server=_Srv(), request=REQ, layer="L",
                             result={"ok": True, "published": True, "outcome": "create"}) is None
    # a no-op / review result (no inverse, not published) -> not recorded
    assert change_log.record(db, server=_Srv(), request=REQ, layer="L",
                             result={"ok": True, "published": True, "outcome": "no_op"}) is None
...
def test_record_persists_published_change_with_inverse(db):
    row = change_log.record(db, server=_Srv(), request=REQ, layer="Network", ticket_id="INC42",
                            actor="user:khalid",
                            result={"ok": True, "published": True, "outcome": "create", "inverse": _INV,
                                    "source_object": "h-x", "destination_object": "Any"})
...
def test_record_stamps_remove_action_and_revoke_summary(db):
    row = change_log.record(db, server=_Srv(), request=REQ, layer="L",
                            result={"ok": True, "published": True, "action": "remove", "outcome": "deny",
...
def test_mark_reverted_then_clears_any_prior_error(db):
    row = change_log.record(db, server=_Srv(), request=REQ, layer="L",
                            result={"ok": True, "published": True, "outcome": "disable",
